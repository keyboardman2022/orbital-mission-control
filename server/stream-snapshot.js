...
function buildSnapshot(stream,data,{selectedId,now=Date.now()}={}){
  ...
  for(const record of data.satellites){
    ...
    const changed=!old||old.tick!==record.state?.tick||old.status!==record.status;
    if(!changed)continue;
    const immediate=!old||old.status!==record.status;
    if(!immediate&&record.id!==selectedId&&now-old.sentAt<1000)continue;
    satellites.push(!old?record:{id:record.id,state:record.state,status:record.status,endReason:record.endReason,birthTick:record.birthTick,telemetry:record.telemetry});
    stream.known.set(record.id,{tick:record.state?.tick,status:record.status,sentAt:now});
  }
