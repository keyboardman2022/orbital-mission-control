...
const engine=new Engine({filename:workerData.filename,now:()=>wallOrigin+performance.now()-monoOrigin,maxActive:workerData.maxActive,velocityRelativeTolerance:workerData.velocityRelativeTolerance});
engine.interruptExports();
let lastCommit=performance.now(),lastArchive=performance.now(),lastWall=Date.now(),lastMono=performance.now();
const timer=setInterval(()=>{
  try{
    ...
    if(mono-lastArchive>=1000&&engine.tick>=86400*M.MODEL.tickRate&&engine.health().lagSeconds<.5){engine.archive();lastArchive=performance.now();}
