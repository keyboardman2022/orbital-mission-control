...
class Engine{
  constructor({filename,now=Date.now,maxActive=1000}){
  ...
  health(){return {tick:this.tick,lagSeconds:Math.max(0,this.targetTick()-this.tick)/M.MODEL.tickRate,status:this.clockError?'clock_error':this.targetTick()-this.tick>120?'recovering':'running',active:this.records.size,clockError:this.clockError};}
  session(userId){
  ...
  insertPoint(id,p){this.db.prepare('INSERT INTO points VALUES (?,?,?,?,?,?,?,?,?)').run(id,p.seq,p.tick,p.elapsedSeconds,p.x,p.y,p.vx,p.vy,p.kind);}
  sample(r,kind){
    const p={...M.snapshot(r.state),kind:kind||(r.state.status==='active'?'sample':r.state.status)};
  ...
  advanceTo(target,{maxSteps=Infinity,budgetMs=Infinity}={}){
    ...
    while(this.tick<target&&steps<maxSteps&&performance.now()-started<budgetMs){
      ...
      for(const r of active){
        M.step(r.state);this.dirty.add(r.id);
        if(r.state.status!=='active'){this.finish(r,r.state.status);continue;}
        const last=r.lastSample,dt=r.state.elapsedSeconds-(last?.elapsedSeconds??0);
        const curved=last&&Math.hypot(r.state.x-last.x-last.vx*dt,r.state.y-last.y-last.vy*dt)>.5;
        if(!last||dt>=1-1e-9||Math.hypot(r.state.x,r.state.y)<4*M.MODEL.rs||curved)this.sample(r);
      }
      this.tick++;steps++;
    }
    if(this.tick>=target){
  ...
  checkpoint(){
    ...
    this.transaction(()=>{
      ...
      for(const e of this.pendingEvents)this.db.prepare('INSERT INTO events VALUES (?,?,?)').run(e.id,e.seq,JSON.stringify(e.event));
      for(const id of this.dirty){const r=this.records.get(id);if(r)this.db.prepare('UPDATE satellites SET record=? WHERE id=?').run(JSON.stringify(r),id);}
      this.db.prepare('INSERT OR REPLACE INTO meta VALUES (?,?)').run('clock',JSON.stringify({epochMs:this.epochMs,tick:this.tick,savedWallMs:this.lastGoodWallMs}));
    });
    this.pending=[];this.pendingEvents=[];this.dirty.clear();
    for(const [id,r] of this.records)if(!['active','queued'].includes(r.status))this.records.delete(id);
  }
  ...
  trajectory(userId,id,query={}){
    const r=this.owned(userId,id);this.checkpoint();
    ...
    const rows=this.db.prepare('SELECT seq,tick,elapsed AS elapsedSeconds,x,y,vx,vy,kind FROM points WHERE satellite_id=? AND seq>? AND tick>=? AND tick<=? ORDER BY seq LIMIT ?').all(id,after,from,to,count+1);
    return {points:rows.slice(0,count).map(p=>({...p,...U.telemetry(p,r.initial.calibration||U.SCALE)})),nextCursor:rows.length>count?rows[count-1].seq:null,cutoffTick:to,
      calibration:r.initial.calibration||U.SCALE,calibrationInferred:!r.initial.calibration};
  }
  terminate(userId,id){
  ...
  prepareExport(userId,id,format){
    ...
    if(r.status==='active'&&r.lastSample?.tick!==r.state.tick){this.records.set(id,r);this.sample(r,'cutoff');}
    this.checkpoint();
