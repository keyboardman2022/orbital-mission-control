...
const record=(tick=0,status='active',coverageStatus='recording')=>({id:'a',name:'satellite',massKg:1000,initial:{name:'satellite'},birthTick:0,state:{tick,x:tick,y:0,vx:1,vy:0},status,endReason:status==='active'?null:status,telemetry:{speed:1},trajectoryCoverage:{status:coverageStatus,lastSeq:coverageStatus==='capped'?10:9,endTick:tick,endElapsedSeconds:tick/240,truncated:coverageStatus==='capped'}});
...
test('stream publishes a trajectory cap even when the physical state tick is unchanged',()=>{
 const state=api.createStreamState();api.buildSnapshot(state,{satellites:[record(240)]},{now:0});
 const capped=api.buildSnapshot(state,{satellites:[record(240,'active','capped')]},{now:1,selectedId:'a'}).satellites[0];
 assert.equal(capped.trajectoryCoverage.status,'capped');
 assert.equal(capped.trajectoryCoverage.truncated,true);
});
